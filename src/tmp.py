...
def coarse_segmentation(filename):
    path, name = os.path.split(filename)
    output_dir = os.path.join(path, name[:-4])
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    os.makedirs(output_dir)
    os.mkdir(os.path.join(output_dir, 'mediumsizedSegments'))
    os.mkdir(os.path.join(output_dir, 'overSegments'))
    os.mkdir(os.path.join(output_dir, 'touchingCases'))

    im = cv2.imread(filename)
    image = preprocess_image(im)
    cv2.imwrite(os.path.join(output_dir, name[:-4]+"_bin.png"), convert_binary_to_normal_im(image))
    
    averageStroke = int(stroke_width_transform.scrub(filename))
    print("Stroke Width: {}".format(averageStroke))
    kernel = np.ones((8,1), np.uint8)
    closingImg = cv2.morphologyEx(img_as_uint(image), cv2.MORPH_CLOSE, kernel)
    cv2.imwrite(os.path.join(output_dir, name[:-4]+"_closing.png"), ~closingImg)
    closingImg = closingImg > 0

    overSegments, mediumSegments = check_touching_image(closingImg, image)
    [cv2.imwrite(os.path.join(output_dir, "overSegments/oversized_segments_{}.png".format(i)), 
        convert_binary_to_normal_im(overSegments[i])) for i in range(len(overSegments))]
    [cv2.imwrite(os.path.join(output_dir, "mediumsizedSegments/mediumsized_segments_{}.png".format(i)), 
    ...
    for index in range(len(overSegments)):
        ...
        [cv2.imwrite(os.path.join(output_dir, "mediumsizedSegments/untouchingCases_{}_{}.png".format(index,i)), 
            convert_binary_to_normal_im(untouchingCases[i])) for i in range(len(untouchingCases))]
        [cv2.imwrite(os.path.join(output_dir, "touchingCases/touchingCases_{}_{}.png".format(index,i)), 
...
if __name__ == '__main__':
    ...
    # path = "/home/thanh/cinnamon/toppan/touching-character module/segmentation/samples/sample1.png"
    # image = cv2.imread(path)
